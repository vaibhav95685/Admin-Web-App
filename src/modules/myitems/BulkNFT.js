...
const BulkNFT = ({ createBulkNFt, setCreateBulkNFT }) => {
  ...
  const validateFields = (name, value) => {
    if (value === "")
      return setNftError({ ...nftError, [name]: "This Field is Required" });
  };
  ...
  const createNftHandler = async (data) => {
    ...
    let ethContract = "0x40ED9c272908a9D39Fc0E6Cd49D10263302D3524";
    ...
    if (data?.blockchain === "Polygon") {
      ...
      let tokenId=Utils.generateRandomNumber();
    ...
    if (data.contractAddress.length > 0) {
      ...
      if (blockchainRes) {
        setLoaderState(false);
        toast.success("Batch NFT Created");
      }
      console.log("blockchainError", blockchainError);
      console.log("blockchainResult", blockchainResult);
    }

    let savedNftJSon = getRequestDataForSaveNftContent(
      tokenId,
      data,
      blockchainRes
    );
  ...
  return (
    <>
      ...
      >
        ...
        {loaderState && (
          <MintPopup
            logoCdn={logoCdn == "" ? previewLogoImage : logoCdn}
            name={nftData.name}
        ...
        <div className="addStoreItem">
          <div className="addFeaturedAssestsContainer">
            <HeadingTittle>Create Bulk Mint NFT</HeadingTittle>
            <div
              className=" confirmationModalHeading confirmationModalCross"
              onClick={() => setCreateBulkNFT(false)}
            >
              X
            </div>
          </div>
          ...
          <Fields>
              ...
              >
                ...
                {collections.length &&
                  collections.map((item) => (
                    <option
                      value={`${item.name}__${item.blockchain}__${item?._id}`}
                    >
                      {item.name}
                    </option>
                  ))}
            ...
            <ContainerSecond>
              ...
              <SelectField
                disabled={
                  nftData.collection === "Anfto Collection" ? false : true
                }
                name="blockchain"
                onChange={(e) => handleInputChange(e)}
              >
                <option
                  disabled
                  selected={collectionBlockchain === "" ? true : false}
                >
                  Select Blockchain
                </option>
                {blockchainOption.length &&
                  blockchainOption.map((item) => (
                    <option
                      selected={
                        collectionBlockchain === item.name ? true : false
                      }
                      value={item?.name}
                    >
                      {item.name}
            ...
            <ContainerFirst>
              ...
              ></InputField>
              {nftError.name && (
                <small style={{ color: "#ff5050" }}>{nftError.name}</small>
              )}
            </ContainerFirst>
            ...
            <ContainerFirst>
              ...
              ></InputField>
              {nftError.price && (
                <small style={{ color: "#ff5050" }}>{nftError.price}</small>
              )}
            </ContainerFirst>
            ...
            <ContainerFirst>
              ...
              ></InputField>
              {nftError.royalty && (
                <small style={{ color: "#ff5050" }}>{nftError.royalty}</small>
              )}
            </ContainerFirst>
