...
import checkOption from "../../assets/images/checkOption.svg";
...
const MyItemsNft = ({ store }) => {
  ...
  useEffect(async () => {
    await getNfts(minValue, maxValue, sort, setNftsData, store.token);
  ...
  const handlePriceFilter = async () => {
    ...
    //   toast.info('Please fill all the fields.');
    // } else {
    //   await getNfts(minValue, maxValue, sort, setNftsData);
    //   setPriceRangeBox(false);
    //   // await getNfts(saleType, minValue, maxValue, sort, setNftsData);
    // }
  };
  ...
  const handleClearPriceFilter = async () => {
    ...
    setPriceRangeBox(false);
    setPriceRangeBox2(false);
    await getNfts("", "", sort, setNftsData, store.token);
  };
  ...
  return (
    <>
      <div className="billingPeriodContainer" style={{justifyContent: 'start'}}>
        <div className="billingPeriod--active">User NFTs</div>
      ...
      <div className="sortingContainer">
        <div className="saleTypeandPricingContainer ">
          ...
          <div className="priceRangeMobile">
            <div
              className="saleTypeandPricing pricingDropDown"
              onClick={() => {
...
export default connect(mapStateToProps)(MyItemsNft);
